...
function showAddValve(fromVisual = false) {
    ...
    console.log('showAddValve called, activeProjectId=', activeProjectId);
    if (!activeProjectId) {
...
function renderDetailsList() {
    ...
    // Ensure action buttons are present under project title (fix for missing buttons)
    ...
    if (actionsEl) {
        actionsEl.innerHTML = `
            ...
            <button class="btn btn-primary" onclick="showVisual()" style="width:auto; padding:8px 12px;">📊 Näytä kaavio</button>
            <button class="btn btn-secondary" onclick="showBalance()" style="width:auto; padding:8px 12px;">⚖️ Tasapaino</button>
            <button class="btn btn-secondary" onclick="optimizeEnergy()" style="width:auto; padding:8px 12px;">⚡ Optimoi</button>
            <button class="btn btn-secondary" onclick="exportReport()" style="width:auto; padding:8px 12px;">📄 Raportti</button>
            <button class="btn btn-secondary" onclick="showSettings()" style="width:auto; padding:8px 12px;">⚙️ Asetukset</button>
        `;
    }
...
function showAddDuct(){
    console.log('showAddDuct called, activeProjectId=', activeProjectId);
    if (!activeProjectId) {
...
function saveDuct() {
    ...
    const name = document.getElementById('ductName').value;
    const size = document.getElementById('ductSize').value;
    
    if (!name) return alert("Anna rungolle nimi!");
    ...
    saveData();
    showView('view-details'); // Palaa etusivulle
    renderDetailsList();      // Päivitä lista
}
